...
bool AssemblerUI(vector<Program *> &AsmPrograms, RamVector &MemoryImage)
{
	...
	sprintf(sMessageBuffer, "\t\tLC-3 Assembler %.15s, Ashley Wise", ASM_VER);
	...
	//Check to see if we should run the translator on the command-line DEFEINEs
	if(Flags.fOldLC3)
	{
		for(list< pair<string, string> >::iterator DefineIter = DefineList.begin(); DefineIter != DefineList.end(); DefineIter++)
		{
			string sTemp;
			if(!AsmConvertLC3Line(DefineIter->second, sTemp, LocationVector(), AsmCallBack))
				return false;
	...
	for(i = 0; i < InputList.size(); i++)
	{
		...
		AsmPrograms.push_back(new Program(LocationStack, InputList[i], LC3ISA::Addressability));
	}
	...
	for(i = 0; i < AsmPrograms.size(); i++)
	{
		...
		if(ToLower(AsmPrograms[i]->sFileName.Ext) == "obj" || ToLower(AsmPrograms[i]->sFileName.Ext) == "bin")
		{
			...
			if(!Assemble<LC3ISA>::Disassemble(AsmFile, *AsmPrograms[i], AsmCallBack))
				fRetVal = false;
		}
		else
		{
			...
			if(Flags.fOldLC3)
			{
				...
				if(!AsmConvertLC3(AsmFile, Asm3File, AsmPrograms[i]->LocationStack, AsmCallBack))
				{
					fRetVal = false;
					continue;
				}
			...
			if(!Assemble<LC3ISA>::Compile(AsmFile, *AsmPrograms[i], AsmCallBack))
				fRetVal = false;
		}
	}	
	if(!fRetVal)
	...
	fRetVal = Assemble<LC3ISA>::Link(AsmPrograms, MemoryImage, AsmCallBack);
	if(!fRetVal)
		goto CleanUp;
	...
	if(Flags.fOutputVHDL)
	{
		...
		AsmCallBack(Info, sMessageBuffer, LocationVector());
		Assemble<LC3ISA>::VHDLWrite(VHDLFile, MemoryImage, AsmCallBack);
	}
	else
	...
	{
		cout << endl;
		Assemble<LC3ISA>::VHDLWrite(cout, MemoryImage, AsmCallBack);
		cout << endl;
	}
