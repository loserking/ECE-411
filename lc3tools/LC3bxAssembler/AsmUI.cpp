...
#include "../LC3bxAssembler/LC3bxISA.h"

using namespace std;
using namespace JMT;
using namespace Assembler;
using namespace LC3bx;
...
bool AssemblerUI(vector<Program *> &AsmPrograms, RamVector &MemoryImage)
{
	...
	sprintf(sMessageBuffer, "\t\tLC-3bx Assembler %.15s, Ashley Wise", ASM_VER);
	...
	for(i = 0; i < InputList.size(); i++)
	{
		...
		AsmPrograms.push_back(new Program(LocationStack, InputList[i], LC3bxISA::Addressability));
	}
	...
	for(i = 0; i < AsmPrograms.size(); i++)
	{
		...
		if(ToLower(AsmPrograms[i]->sFileName.Ext) == "obj" || ToLower(AsmPrograms[i]->sFileName.Ext) == "bin")
		{
			...
			if(!Assemble<LC3bxISA>::Disassemble(AsmFile, *AsmPrograms[i], AsmCallBack))
				fRetVal = false;
		}
		else
		{
			...
			if(!Assemble<LC3bxISA>::Compile(AsmFile, *AsmPrograms[i], AsmCallBack))
				fRetVal = false;
		}
	}	
	if(!fRetVal)
	...
	fRetVal = Assemble<LC3bxISA>::Link(AsmPrograms, MemoryImage, AsmCallBack);
	if(!fRetVal)
		goto CleanUp;
	...
	if(Flags.fOutputVHDL)
	{
		...
		AsmCallBack(Info, sMessageBuffer, LocationVector());
		Assemble<LC3bxISA>::VHDLWrite(VHDLFile, MemoryImage, AsmCallBack);
	}
	else
	...
	{
		cout << endl;
		Assemble<LC3bxISA>::VHDLWrite(cout, MemoryImage, AsmCallBack);
		cout << endl;
	}
